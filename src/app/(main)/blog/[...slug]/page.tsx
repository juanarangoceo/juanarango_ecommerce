...
function CategoryPage({ categorySlug, meta, posts, prompts }: { categorySlug: string; meta: { title: string; description: string }; posts: any[]; prompts?: any[] }) {
  return (
    <main className="container mx-auto px-4 py-20 min-h-screen">
      <div className="mb-12">
        <Link href="/blog" className="inline-flex items-center text-sm font-medium text-white/45 hover:text-white transition-colors mb-6">
          <ArrowLeft className="w-4 h-4 mr-2" /> Volver al Blog
        </Link>
        <h1 className={`text-4xl font-extrabold tracking-tight lg:text-5xl mb-4 ${categorySlug === 'prompts' ? 'text-transparent bg-clip-text bg-gradient-to-r from-primary to-pink-600' : ''}`}>
          {meta.title}
          {categorySlug === 'prompts' && <Zap className="inline-block w-8 h-8 md:w-10 md:h-10 ml-3 text-primary align-baseline" />}
        </h1>
        <p className={`text-xl max-w-2xl ${categorySlug === 'prompts' ? 'text-primary/80' : 'text-muted-foreground'}`}>{meta.description}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8">
          {posts && posts.length > 0 ? (
            ...
            <div className="text-center py-20">
              ...
              <p className="text-white/45">Pronto publicaremos contenido sobre {meta.title.toLowerCase()}.</p>
            </div>
          )}
        </div>
        <aside className="lg:col-span-4 space-y-6">
          <div className="lg:sticky lg:top-24 space-y-6">
            <NitroCtaCard />
            <div className="bg-[#0d110e]/60 border border-white/10 rounded-xl p-6"><NewsletterForm /></div>
          </div>
        </aside>
      </div>
    </main>
    ...
    <>
      ...
      <div className="bg-[#0b0e0c] min-h-screen">
        ...
        <header className="container mx-auto px-4 pt-32 pb-12 max-w-5xl">
            ...
            <nav className="flex items-center gap-2 text-sm text-white/45 mb-6">
              ...
              {breadcrumb && (
                <>
                  <span>/</span>
                  <Link href={`/blog/${post.category}`} className="hover:text-white transition-colors">
                    {breadcrumb.title}
                  </Link>
                </>
              )}
            ...
            <h1 className="text-3xl md:text-6xl font-extrabold tracking-tight text-white mb-6 md:mb-8 leading-[1.1]">
              {post.title}
            </h1>
            ...
            <div className="flex flex-wrap gap-6 items-center text-sm text-white/58 border-t border-b border-white/9 py-6">
                <div className="flex items-center gap-2">
                    <User className="w-4 h-4" />
                    <span className="font-semibold text-white/84">{authorName}</span>
                </div>
                <div className="w-px h-4 bg-[#111512]" />
                <div className="flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                ...
                <article className="lg:col-span-8">
                    ...
                    {post.mainImage?.asset?._ref && (
                        <div className="mb-6 rounded-lg overflow-hidden border border-white/10 shadow-sm relative aspect-video">
                            <Image 
                    ...
                    {post.youtubeVideo?.url && (
                        <div className="mb-8 md:mb-12">
                            ...
                            {post.youtubeVideo.summary && (
                                <p className="text-base text-white/58 mt-4 italic border-l-4 border-primary pl-4 py-1 bg-[#0d110e]/60 rounded-r-lg">
                                    <span className="font-semibold block not-italic text-white/84 mb-1">En este video:</span>
                                    {post.youtubeVideo.summary}
                                </p>
                            )}
                        </div>
                    ...
                    {post.content && (
                        <div className="lg:hidden mb-8 space-y-4">
                            <div className="rounded-lg border border-white/10 bg-[#0d110e] p-4">
                    ...
                    <div className="prose prose-zinc dark:prose-invert prose-lg max-w-none prose-headings:font-bold prose-headings:tracking-tight prose-headings:text-white prose-h2:text-3xl prose-h2:mt-12 prose-h2:mb-6 prose-h2:pb-4 prose-h2:border-b prose-h2:border-white/9 prose-p:leading-relaxed prose-p:text-white/78 prose-strong:text-white prose-strong:font-semibold prose-blockquote:border-l-4 prose-blockquote:border-primary prose-blockquote:bg-[#0d110e] prose-blockquote:py-2 prose-blockquote:px-6 prose-blockquote:not-italic prose-blockquote:rounded-r-lg prose-ul:list-disc prose-ul:pl-6 prose-li:marker:text-primary ">
                            ...
                            <ReactMarkdown
                                ...
                                components={{
                                    h1: ({node, ...props}) => {
                                        ...
                                        return <h2 id={id} className="text-3xl font-bold mt-12 mb-6 pb-2 border-b border-white/10 text-white" {...props}>{props.children}</h2>;
                                    },
                                    h2: ({node, ...props}) => {
                                        const text = String(props.children);
                                        const id = slugify(text);
                                    ...
                                    h3: ({node, ...props}) => {
                                        const text = String(props.children);
                                        const id = slugify(text);
                                        return <h3 id={id} className="text-2xl font-bold mt-8 mb-4 text-white" {...props}>{props.children}</h3>;
                                    },
                                    p: ({node, ...props}) => {
                                        const content = typeof props.children === 'string' 
                                            ? renderTextWithLinks(props.children, linkDictionary, linkedKeywordsSet) 
                                            : props.children;
                                        return <p className="mb-6 text-lg leading-relaxed text-white/78" {...props}>{content}</p>;
                                    },
                                    ul: ({node, ...props}) => {
                     ...
                     {post.faq && post.faq.length > 0 && (
                        <div className="mt-12 md:mt-16 pt-8 md:pt-12 border-t border-white/10">
                          <h2 className="text-2xl md:text-3xl font-bold mb-6 md:mb-8 text-white">Preguntas Frecuentes</h2>
                    ...
                    <div className="mt-12 mb-12">
                      ...
                      <div className="mt-8 mb-4">
                          <a 
                              ...
                              className="group relative overflow-hidden flex items-center gap-4 w-full p-5 sm:p-6 rounded-2xl bg-gradient-to-r from-[#0d110e] to-[#111512]/80 border border-white/15 hover:border-[#2AABEE]/50 transition-all shadow-sm hover:shadow-[0_8px_30px_-6px_rgba(42,171,238,0.25)]"
                          >
                              <Image
                              ...
                              <div className="flex flex-col text-left flex-1 min-w-0">
                                  <span className="font-bold text-white text-[15px] sm:text-base leading-tight mb-1">
                                      ¿Te gustó este contenido?
                                  </span>
                                  <span className="text-[13px] sm:text-sm text-white/58 font-medium tracking-tight">
                          ...
                          <div className="space-y-3 md:space-y-4">
                             {(post.relatedPosts && post.relatedPosts.length > 0 ? post.relatedPosts : post.fallbackPosts).map((related: any, index: number) => (
                                <Link 
                                  ...
                                  className="group block p-4 md:p-5 rounded-lg border border-white/10 hover:border-primary hover:bg-[#0d110e] transition-all duration-200"
                                >
                                   <div className="flex items-start gap-3">
                                      ...
                                      <div className="flex-1 min-w-0">
                                         <h4 className="font-semibold text-white group-hover:text-primary transition-colors text-sm md:text-base line-clamp-2 mb-1">
                                            {related.title}
                                         </h4>
                                         {related.publishedAt && (
                                      ...
                                      <ArrowRight className="w-4 h-4 text-white/58 group-hover:text-primary group-hover:translate-x-1 transition-all flex-shrink-0" />
                                   </div>
                                </Link>
                             ))}
                          </div>
                        ...
                        {/* Table of Contents Box - First Priority */}
                         {post.content && (
                            <div className="space-y-4">
                                <div className="p-6 bg-[#0d110e] rounded-lg border border-white/10">
                                    <TableOfContents toc={parseTOC(post.content)} />
                                </div>
                            </div>
                         )}
