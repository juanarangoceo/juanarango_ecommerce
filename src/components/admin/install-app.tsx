...
// Botón «Instalar app» del panel. Portado de Todópolis y Nitro Bot.
...
export function InstallAppButton({ className = '', children }: { className?: string; children: ReactNode }) {
  ...
  return (
    <>
      ...
      {showHelp && (
        <div
          className="fixed inset-0 z-[70] flex items-end bg-black/60 backdrop-blur-sm p-3 sm:items-center sm:justify-center"
          role="presentation"
        ...
        >
          <section
            ...
            className="w-full rounded-3xl border border-white/10 bg-[#131613] p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] shadow-xl sm:max-w-sm"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="mb-4 flex items-start justify-between gap-4">
              <div>
                <h2 id="install-title" className="text-lg font-semibold tracking-[-0.02em] text-white">
                  Instala el panel
                </h2>
              ...
              <button
                type="button"
                onClick={() => setShowHelp(false)}
                className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-white/[0.06] text-white/70"
                aria-label="Cerrar"
              >
                <X className="h-4 w-4" />
              </button>
            ...
            <div className="space-y-3 text-sm text-white/75">
              <div className="rounded-xl bg-white/[0.04] p-4">
                <p className="font-semibold text-white">En iPhone</p>
                <p className="mt-1">
              ...
              <div className="rounded-xl bg-white/[0.04] p-4">
                <p className="font-semibold text-white">En Android</p>
                <p className="mt-1">
