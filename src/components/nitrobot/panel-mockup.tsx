...
export function PanelMockup() {
  return (
    <div className="rounded-2xl border border-white/10 bg-[#0d110e] overflow-hidden shadow-2xl shadow-black/50 text-left">
      {/* Barra de navegador */}
      <div className="flex items-center gap-2 px-4 py-3 bg-[#0b0e0c] border-b border-white/5">
        <span className="w-3 h-3 rounded-full bg-white/10" />
        <span className="w-3 h-3 rounded-full bg-white/10" />
        <span className="w-3 h-3 rounded-full bg-white/10" />
        <span className="ml-3 font-dm-mono text-[11px] text-white/45 truncate">panel.nitrobot — Conversaciones</span>
      </div>
      ...
      <div className="p-4 sm:p-5 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <div key={m.label} className="rounded-xl bg-[#0b0e0c]/80 border border-white/5 px-3 py-3">
              <p className="text-xl sm:text-2xl font-bold text-white">{m.value}</p>
              <p className="font-dm-mono text-[10px] uppercase tracking-wider text-white/45 mt-1">{m.label}</p>
            </div>
          ))}
        </div>

        {/* Lista de conversaciones en vivo */}
        <div className="rounded-xl bg-[#0b0e0c]/80 border border-white/5 divide-y divide-white/5">
          {conversations.map((c) => (
            <div key={c.name} className="flex items-center gap-3 px-3.5 py-3">
              <div className="w-8 h-8 rounded-full bg-[#111512] flex items-center justify-center text-[11px] font-bold text-white/78 shrink-0">
                {c.name.charAt(0)}
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold text-white truncate">{c.name}</p>
                <p className="text-xs text-white/58 truncate font-dm-mono">
                  {c.state === "typing" ? <TypingDots className="text-primary" /> : c.last}
                </p>
              </div>
              <div className="flex flex-col items-end gap-1 shrink-0">
                <span className="font-dm-mono text-[10px] text-white/45">{c.time}</span>
                {c.state === "humano" ? (
