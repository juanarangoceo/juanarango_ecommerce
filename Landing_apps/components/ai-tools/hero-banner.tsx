...
import { getAppUrl } from "@/lib/ai-tools-data"

export function HeroBanner({ app }: { app: AppTool }) {
  return (
      ...
      <div className="relative flex flex-col gap-6 p-6 md:p-10">
        ...
        <div className="flex items-start gap-5">
          ...
          <div className="flex flex-col gap-2">
            ...
            <p className="max-w-xl text-sm text-muted-foreground leading-relaxed md:text-base">
              {app.longDescription.slice(0, 180)}...
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button variant="outline" size="lg" asChild>
            <Link href={`/ai-tools/${app.id}`}>Ver Detalle</Link>
          </Button>
        </div>
      </div>
    </section>
